// app/api/pikett-assignments/stats/route.ts
...
// GET - Aggregate pikett assignment statistics by status, user, and team
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const allAssignments = await prisma.pikettAssignment.findMany({
      where,
      include: {
        pikett: {
          select: {
    ...
    if (teamId) {
      assignments = allAssignments.filter(a => a.pikett.teamId === teamId);
    }
    ...
    return NextResponse.json(
      { error: 'Failed to fetch pikett stats' },
      { status: 500 }
    );
  }
}
